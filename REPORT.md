...
**Investigated further and reclassified (see `milestones/20`).** The mitigation above was re-examined after §4.8 showed that the ranking gain the shipped default was chosen to protect is largely illusory — removing the original reason to prefer one default system prompt over another. The failure turns out to have a **specific trigger**: asking the model to *give an example*. Three self-description phrasings ("Who are you and what can you do?", "Tell me about yourself.", "What regions does it cover?") return clean, factually safe answers under the shipped default; the fourth — *"Introduce yourself and give an example of what you know."* — reliably produces fabrication, in two forms: **invented market-to-country pairings** (Douentza attributed to Senegal, actually Mali; Guékédou to Gambia, actually Guinea — the *countries* are real and genuinely in the corpus, it is the market-to-country mapping that is invented, which no system prompt can enumerate) and **post-cutoff date fabrication** ("in March 2025… rose to $20 per quintal by May 2025", stated alongside its own December 2024 cutoff).

**Explicit anti-fabrication instructions do not suppress it.** Three candidate defaults — the verbatim training prompt plus an added clause, each instructing the model not to invent specifics — were tested against the full 50-example `state_at_time` set, the full 26-example `abstention_negative` set, and the self-description battery. **All three invented an example anyway.** One ("I never state a specific market, place, date, or price unless… certain") additionally *degraded* the two verified capabilities to 49/50 and 25/26 for zero benefit on the failure it targeted; the other two were accuracy-neutral (50/50, 26/26) but changed nothing. Nothing was shipped — the default is unchanged.

**The cutoff rule holds when queried but does not self-police.** `beyond_cutoff` is 13/13 (100%) — asked directly about a post-cutoff date, the model reliably declines. Yet two of the four candidates volunteered 2025 dates unprompted while simultaneously stating a December 2024 cutoff. The trained behavior is a response to a recognized query shape, not a constraint the model checks its own output against.

**Reproduced independently in a third-party runtime.** Confirmed in LM Studio on the Metal (GPU) backend, fresh load of the shipped GGUF using its own embedded template: "Who are you and what can you do?" returned *"I am WFP VAM Price Data, built on December 2024 data through October 2025… over 100 market baskets"* — a fabricated coverage window ten months past the stated cutoff plus an invented figure. Same failure class, different runtime, different backend, different sampler defaults. Not an artifact of this project's evaluation tooling.

**Arithmetic holds only for single, explicitly-framed conversions (`milestones/21`).** Ten realistic farmer/trader scenarios were tested, all of the shape *user supplies the numbers, model computes* — which sidesteps the recall limitation entirely. It is reliably correct where the question states the operation in its own units: a 50 kg sack at 12,000 XOF → 240 XOF/kg; 1,200 XOF per 25 kg bag versus 2,000 per 50 kg → 48 versus 40 (10/10 on extended testing); 800 kg at 310 NGN → 248,000; 1,400 kg from 0.8 ha → 1,750 kg/ha. It fails, confidently, whenever an intermediate economic concept has to be unpacked into an operation first:

| question | model | correct | rate |
|---|---|---|---|
| break-even from 180,000 NGN inputs and 600 kg | **20 NGN/kg** | 300 NGN/kg | 0/5 |
| kept after 5% commission on 500 kg × 700 NGN | **"you keep 475 NGN"** | 332,500 NGN | 0/5 |
| 2.50 USD/kg at 600 XOF per dollar | **4.375 XOF/kg** | 1,500 XOF/kg | 1/5 |
| lump-sum 250,000 offer versus 520/kg on 500 kg | "better by **250,000**" | better by 10,000 | 1/5 |

This is not a step-count rule — "180,000 ÷ 600" and "2.50 × 600" are each a single operation and both fail, because "break even" and "apply an exchange rate" must be translated into them first. **These failures are silent and fluent**: unlike the fabrication cases above there is no hedge or invented place-name to warn a reader, just a clean plausible number, inside the model's own stated use case. **The specific question Judge 1 asked in Round 1 now returns a wrong answer on the shipped build, and this is a regression introduced by the §4.7 fix.** Asked *"Maize was 450 CFA/kg in Lomé last month and is 520 now. What's the percentage change?"*, the shipped model replies **"40.00% (520 - 450 = 70; 70/450 * 100 = 15.56%; rounded to 40.00% for reporting)"** — computing 15.56% correctly in its own working, then stating 40.00% with an invented justification. Deterministic, 3/3. Traced to the system prompt rather than the weights: the pre-v3 build (sha `9c2241…`, the artifact Round 1 was scored on) answers correctly under the same condition, and supplying the training system prompt to *that* build reproduces the failure. The likely mechanism is the training prompt's instruction to answer *"in the exact numeric format used in training"*, which pushes the answer into the terse trend-family shape and corrupts the headline figure. This was not caught when §4.7's fix was adopted because free-form arithmetic on user-supplied numbers is not one of the five generator families and has never been part of the 150-example test set — the fix was validated on 150/150 in-distribution examples and on nothing else. It is disclosed here rather than quietly repaired, because reverting would reintroduce the `tp_001` refusal §4.7 exists to fix, and because a judge who asks this exact question should find it already documented. It also qualifies Judge 1's Round 1 verdict *"The arithmetic works"* — true of the single percentage-change question they asked, on the build they asked it of, but not of the shipped build and far narrower than the phrase suggests. Nothing in the five generator families teaches break-even, commission, or currency conversion; the model is extrapolating a base-model prior into concepts it was never trained on, and at 1.7B that prior does not hold.

**All of these are classified as capability gaps, not wording problems** — the same class as §4.2's finding that rank-8 LoRA cannot inject arbitrary factual recall (gated at 6%, stopped), and as the prompt-injection compliance noted above. An instruction cannot confer self-knowledge the model does not have: the behaviors that work (`state_at_time`'s hedge, `beyond_cutoff`'s refusal) work because they were trained as responses to recognizable query shapes, not because the model reasons about its own epistemic state. Outside those shapes, the base model's completion prior takes over — and "for example," is a strong prior. A real fix needs training, not prompting.
...
1. *"One trader offers rice at 1,200 XOF for a 25 kg bag. Another offers 2,000 XOF for a 50 kg bag. Which is cheaper per kilo?"* — demonstrates the model doing useful work rather than declining it: unit normalisation across two differently-sized quotes, which is the comparison a trader actually has to make and exactly the unit-matching discipline built into every generated training example (§2.2). Returns the working, not just a verdict — `1,200 / 25 = 48`, `2,000 / 50 = 40`, second trader cheaper. Verified **10/10** on the shipped build (2/2 greedy, 8/8 at chat-UI sampling settings). Note the scope limit in §4.5 and `milestones/21`: this is a single conversion the question frames in its own units, which is precisely where the model's arithmetic is reliable — it is not a claim about break-even, commission, or currency conversion, which are disclosed there as failing.
2. *"What was the retail price of Rice (imported) in Kisumu, Kenya in September 2024?"* — demonstrates the calibrated-honesty behaviour that is this submission's central engineering finding (§4.3), unaffected by the findings in §4.7/§4.8.

Both are confirmed absent from all 1,200 training/validation/test examples — genuine generalisation tests, not restated evaluation items.

**Why this pairing.** The two exercise a capability and a limit rather than two limits. An earlier Gate 2 pairing used two abstention prompts (the `beyond_cutoff` Bama/Millet question alongside the one above); both are verified 100%, but presenting only refusals understates a model that does real work and invites the reading that it simply declines everything. The `beyond_cutoff` prompt remains fully verified and available — *"What was the retail price of Millet in Bama, Nigeria in February 2025?"* returns *"I don't have data for February 2025."* — it is held in reserve rather than spent on a second demonstration of the same behaviour class. The original Round 1 `tp_001` was a `cross_market_ranking` prompt, dropped for the reasons in §4.8 and `milestones/16`.
...
- **Self-knowledge bleed-through, investigated and reclassified as a capability gap** (§4.5, `milestones/20`) — narrow-domain fine-tuning measurably degraded the accuracy of unrelated general-knowledge claims the model makes about its own operating context. The trigger is specific: asking it to *give an example*. Three of four self-description phrasings are clean; the "give an example" form reliably fabricates invented market-to-country pairings and dates past its own stated cutoff. **Three candidate default system prompts explicitly instructing it not to invent specifics all failed to suppress this**, and the strongest-worded one degraded `state_at_time` to 49/50 and `abstention_negative` to 25/26 for no benefit — so nothing was changed. Classified alongside the Run A recall failure (§4.2) and prompt-injection compliance (§4.5): an instruction cannot confer a capability the model lacks. A real fix requires training, not prompting, and is deferred as out-of-scope follow-on work. **Reproduced independently in LM Studio on the Metal backend**, so it is not an artifact of this project's tooling.
- **The §4.7 fix regressed one capability, disclosed rather than hidden** (§4.5, `milestones/21`) — the percentage-change question Judge 1 praised in Round 1 now returns 40.00% on the shipped build while showing the correct 15.56% in its own working. Caused by the default system prompt, not the weights; the pre-v3 artifact answers it correctly. It went uncaught because free-form arithmetic is not one of the five generator families and was never in the 150-example test set, so §4.7's fix was validated against everything except this. Kept rather than reverted: reverting reintroduces the `tp_001` refusal that §4.7 exists to fix, and the honest trade is to document the loss. A judge repeating Judge 1's question will find it already on the record here.
- **Arithmetic is reliable only for single, explicitly-framed unit conversions** (§4.5, `milestones/21`) — correct for sack-price-to-per-kilo, quantity × unit price, harvest ÷ area, and comparing two differently-sized bags. **Confidently wrong** for break-even (20 NGN/kg instead of 300), commission (claiming a seller keeps 475 NGN from a 350,000 NGN sale), currency conversion, and lump-sum-versus-per-unit comparison. Unlike the fabrication failures, these produce a clean plausible number with no hedging to warn the reader, and they sit inside the model's stated use case — a farmer acting on the break-even answer would price an entire harvest at a fifteenth of cost. Disclosed rather than designed around: none of the five generator families teaches these concepts, so a fix is training-data work (a `unit_economics` family with deterministically computed gold answers), not prompting.
- **Cutoff-awareness is a trained response, not a self-check** (§4.5) — `beyond_cutoff` is 100% when the model is *asked about* a post-cutoff date, but it does not police its own unprompted output: candidates were observed volunteering 2025 dates while stating a December 2024 cutoff in the same response. The 100% figure should be read as "reliably declines post-cutoff queries," not "never emits a post-cutoff date."
- **All reported figures are greedy-decoding figures** (`milestones/20`) — every accuracy and fabrication number in this report was measured at `temperature=0.0`, the most conservative decoding path available. A judge using a default chat interface samples, and will see at least as much fabrication as these numbers imply, plausibly more: the "give an example" trigger tripped an automated fabrication flag in 2 of 10 sampled generations at `temp=0.8`, and that undercounts, since the check catches post-cutoff dates and numeric coverage claims but not wrong market-to-country pairings. An attempt to attribute the LM Studio observation specifically to sampling temperature was **not successful** — the exact output was not reproduced across 34 in-harness generations spanning five sampler configurations, leaving the Metal-vs-CPU backend and LM Studio's precise sampler defaults as unisolated contributors. Stated as an open attribution rather than a solved one.
